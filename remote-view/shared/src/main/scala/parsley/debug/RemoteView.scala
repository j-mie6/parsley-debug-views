...
final case class RemoteView private (port: Int, address: String, debugName: Option[String]) extends DebugView.Reusable with DebugView.Pauseable with DebugView.Manageable {
    ...
    private def makeRequest[R: up.ReadWriter](endpoint: sttp.model.Uri, payload: String, timeout: FiniteDuration) = {
        basicRequest.readTimeout(timeout)
    ...
    override private [debug] def renderManage(input: =>String, tree: =>DebugTree, refs: CodedRef*): (Int, Seq[CodedRef]) = {
        val resp = renderWithTimeout(input, tree, BreakpointTimeout, isDebuggable = true, refs.toSeq)
        (resp.getSkipsOrDefault, resp.getNewRefsOrDefault)
    }

    private def renderWithTimeout(input: =>String, tree: =>DebugTree, timeout: FiniteDuration, isDebuggable: Boolean = false, refs: Seq[CodedRef] = Nil) = {
...
object RemoteView {
    ...
    private def checkIp(address: String): Boolean = {
        ...
        val addrNumValid: Boolean = numberStrings.forall { (number: String) =>
            number.length > 0 && {
